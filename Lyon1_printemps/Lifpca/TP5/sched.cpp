...
void change_ordonnancement(int policy, int priorite) {
    ...
    // On récupère d'abord la configuration actuelle (juste pour l'afficher si besoin)
    int ret = pthread_getschedparam(tid, &current_policy, &param);
    if (ret != 0) {
        perror("pthread_getschedparam");
        exit(EXIT_FAILURE);
    }

    // On prépare la nouvelle priorité
    param.sched_priority = priorite;

    // On applique la nouvelle politique et la priorité
    ret = pthread_setschedparam(tid, policy, &param);
    if (ret != 0) {
        perror("pthread_setschedparam");
        std::cerr << "Erreur lors du changement d'ordonnancement (êtes-vous root ?)" << std::endl;
        exit(EXIT_FAILURE);
    } else {
        std::cerr << "Changement d'ordonnancement réussi : ";
        if (policy == SCHED_FIFO) std::cerr << "SCHED_FIFO";
        else if (policy == SCHED_RR) std::cerr << "SCHED_RR";
        else if (policy == SCHED_OTHER) std::cerr << "SCHED_OTHER";
        std::cerr << " avec priorité " << priorite << std::endl;
    }
