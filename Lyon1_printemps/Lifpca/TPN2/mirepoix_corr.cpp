...
class GestionPont_V1 : public GestionPont {
  void entree(int masse, int i) {
    ...
    /* À COMPLETER */
    m.lock();
    while (masse + masse_courante > MASSE_MAX) {
      c.wait(m);
    }
    masse_courante += masse;
    m.unlock();
    verif.entree_effective(masse, i);
  }
...
class GestionPont_Famine : public GestionPont {
  void entree(int masse, int i) {
    ...
    // On peut passer si 1) ça ne surcharge pas le pont, 2) on est le
    // premier arrivé parmis ceux qui attendent (condition qui inclue
    // le cas où on est la seule voiture potentiellement en attente).
    while (not(masse + masse_courante <= MASSE_MAX &&
	       FIFO.front() == i)) {
      if (debug){
	printf("Voiture %3d: attente (fifo.front() == %d)\n", i, FIFO.front());
      }
      c.wait(m);
    }
...
double loop_parallel(int *attentes, int *passages, int *masses){
  ...
  /* À COMPLÉTER */
  double results[nb_cars];

  vector<thread> v;
  int i;
  for (i=0; i<nb_cars; i++){
    v.push_back(thread(car_thread, i, attentes[i], passages[i], masses[i], std::ref(results[i])));
  }
  for (thread &t : v) {
    t.join();
  }
